...
> [!NOTE]
> Так как сабы в формате *.ass умеет накладывать и Shortcut и ffmpeg одной командой, то достаточно только реализовать генерацию красирового ass c разбивкой на слова по точным таймингам.
...
| **Лицензия** | **MIT** (свободная, коммерческое использование разрешено). | **BSD-2-Clause / GPL** (свободная, но требует внимания при интеграции в закрытые проекты). | **Собственная** (вы решаете сами, какой код сделать открытым). Примечание: я склоняюсь к GPL v3 т.к. продукту с зактрытым кодом будет тяжко конкурировать с текущем закрытым монополистом. |
...
* **Плюсы:**
  ...
  * **Удобство:** Никакой консоли (примечание редактора: но можно и в консоле, чтобы не парится над реализацией GUI, по крайней мере на этапе MVP) — перетащили видео, подождали 15 секунд, скачали готовый результат.
* **Минусы:**
